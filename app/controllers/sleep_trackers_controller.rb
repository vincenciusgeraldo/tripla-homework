class SleepTrackersController < ApplicationController
  ...
  def record_sleep
    ...
    if sleep_tracker.save
      render json: user_sleep_trackers, status: :created
    else
  ...
  def record_awake
    if sleep_tracker&.awake_at.nil?
      ...
      render json: user_sleep_trackers, status: :ok
    else
  ...
  def user_sleep_trackers
    SleepTracker.where(user_id: current_user.id).order(sleep_at: :desc)
